...
class ControllerCheckoutMultisellerShippingMethod extends Controller {
	...
	function array_sort($array, $on, $order=SORT_ASC)
	{
  	...
  	public function index() {
		...
		if (!empty($shipping_address)) {
			...
			// List all shipping methods for each products (or each seller for combinable)
			...
			foreach ($this->cart->getProducts() as $product) {
						...
						if ($this->MsLoader->MsShipping->getQuote($shipping_address, $shipping_method['geo_zone_id']) || ($shipping_method['geo_zone_id'] == 0)) {
							...
							if (!$skip) {
								...
								if ($shipping_type == MsShipping::SHIPPING_TYPE_FIXED) { // Fixed shipping type
									...
									$cost = $this->currency->format($shipping_method['cost'] * $product['quantity'], $this->config->get('config_currency'), 1);
									...
									$cost = $this->currency->format($product['weight']/$this->weight->convert($shipping_method['weight_step'], $shipping_method['weight_class_id'], $product['weight_class_id']) * $shipping_method['cost_per_unit'], $this->config->get('config_currency'), 1);
				...
				$this->data['products'][] = array(
					...
					'shipping_cost'		=> (isset($shipping_methods[0]) ? $shipping_methods[0]['cost_unformatted'] : "0.0"),
					'name'				=> $product['name'],
					'model'				=> $product['model'],
					'option'			=> $option_data,
				...
				//if ($shipping_methods || !$product['shipping']) {
					$checkout_totals += $this->tax->calculate($this->currency->convert($product['price'], $this->config->get('config_currency'), $this->currency->getCode()), $product['tax_class_id'], $this->config->get('config_tax')) * $product['quantity'];
			...
			if (!empty($combine_sellers)) {
				foreach ($combine_sellers as $unique_seller_id) {
					...
					foreach ($this->data['products'] as $product) {
						if ( ($product['shippable']) && ($product['seller_id'] == $unique_seller_id) ) {
							foreach ($product['shipping_methods'] as $seller_shipping_method) {
								...
								$seller_shipping_method['cost'] = $this->currency->format($cost / $total_seller_products, $this->currency->getCode(), 1);
								...
								$seller_shipping_methods[] = array(
									...
									'cost'									=> $this->currency->format($cost, $this->currency->getCode(), 1),
					...
					$this->data['seller_shipping'][] = array(
						...
						'seller_name'			=> $seller_name,
						'seller_href'			=> $seller_href,
						'shipping_cost'			=> (isset($seller_shipping_methods[0]) ? $seller_shipping_methods[0]['cost_unformatted'] : "0.0"),
			...
			$this->data['checkout_totals'] = $checkout_totals;
			$this->data['checkout_totals_formatted'] = $this->currency->format($checkout_totals, $this->currency->getCode(), 1);
		}
		// <<< THIS (UP THERE) <<<
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/multiseller_shipping_method.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkout/multiseller_shipping_method.tpl';
		} else {
			$this->template = 'default/template/checkout/multiseller_shipping_method.tpl';
		}
	...
	public function jxChangeShippingMethod() {
		...
		//$json['checkout_totals_unformatted'] = $data['checkout_totals'] - $data['shipping_cost_old[' . $product_id . ']'] + $data['shipping_cost_new[' . $product_id . '][' . $data['shipping_method[' . $product_id . ']'] . ']'];
		$json['checkout_totals'] = $this->currency->format($json['checkout_totals_unformatted'], $this->currency->getCode(), 1);
	...
	public function jxChangeSellerShippingMethod() {
		...
		$json['checkout_totals_unformatted'] = $data['checkout_totals'] - $data['seller_shipping_cost_old'][$seller_id] + $data['seller_shipping_cost_new'][$seller_id][$data['seller_shipping_method'][$seller_id]];
		$json['checkout_totals'] = $this->currency->format($json['checkout_totals_unformatted'], $this->currency->getCode(), 1);
	...
	public function validate() {
		...
		// Validate whether shipping address has been set
		$this->load->model('account/address');
