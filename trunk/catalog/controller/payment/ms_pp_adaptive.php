			...
			if (isset($receivers[$seller_id]['ms.paypal']) && !empty($receivers[$seller_id]['ms.paypal']) && filter_var($receivers[$seller_id]['ms.paypal'], FILTER_VALIDATE_EMAIL)) {
				...
				if ($this->MsLoader->MsShipping->getOrderProductShippable($order_product['order_id'], $order_product['product_id'])) {
					if ($shipping_type == MsShipping::SHIPPING_TYPE_FIXED) {
						$receivers[$seller_id]['amount'] += $this->currency->format($this->session->data['shipping_methods'][$order_product['product_id']]['cost_unformatted'], $order_info['currency_code'], 1, false);
					} else if ($shipping_type == MsShipping::SHIPPING_TYPE_COMBINABLE) {
						if (!in_array($seller_id, $sellers)) {
							$receivers[$seller_id]['amount'] += $this->currency->format($this->session->data['seller_shipping_methods'][$seller_id]['cost_unformatted'], $order_info['currency_code'], 1, false);
							$sellers[] = $seller_id;
						}
					}
				}
