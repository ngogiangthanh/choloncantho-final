...
class ControllerSellerAccountProduct extends ControllerSellerAccount {
	...
	public function jxSubmitProduct() {
		...
		if ( ($this->config->get('msconf_download_limit_applies') == 0) || ($this->config->get('msconf_download_limit_applies') == 1 && ( ($this->config->get('msconf_enable_shipping') == 2 && $data['product_enable_shipping'] == 0) || $this->config->get('msconf_enable_shipping') == 1) ) ) {
		...
		if ( $data['product_enable_shipping'] == 1 ) {
			if (isset($data['ms_shipping_methods']) && is_array($data['ms_shipping_methods'])) {
				...
				foreach ($ms_shipping_methods as $ms_shipping_method) {
					if ((!is_numeric($ms_shipping_method['cost'])) || ((float)$ms_shipping_method['cost'] < (float)0)) {
			...
			if ( ($data['shipping_length'] != "" && isset($data['shipping_length'])) && (!is_numeric($data['shipping_length']) || (float)$data['shipping_length'] < (float)0) ) {
				$json['errors']['shipping_dimensions'] = $this->language->get('ms_error_invalid_shipping_dimensions');
			}
			
			if ( ($data['shipping_width'] != "" && isset($data['shipping_width'])) && (!is_numeric($data['shipping_width']) || (float)$data['shipping_width'] < (float)0) ) {
				$json['errors']['shipping_dimensions'] = $this->language->get('ms_error_invalid_shipping_dimensions');
			}
			
			if ( ($data['shipping_height'] != "" && isset($data['shipping_height'])) && (!is_numeric($data['shipping_height']) || (float)$data['shipping_height'] < (float)0) ) {
				$json['errors']['shipping_dimensions'] = $this->language->get('ms_error_invalid_shipping_dimensions');
			}
			
			if ( ($data['shipping_weight'] != "" && isset($data['shipping_weight'])) && (!is_numeric($data['shipping_weight']) || (float)$data['shipping_weight'] < (float)0) ) {
	...
	private function _initForm() {
		...
		$this->data['msconf_download_limit_applies'] = $this->config->get('msconf_download_limit_applies');
